...
func checkAAA(player *[]int) bool{
    ...
    if ((!majunFunc.Contain(playerList,onTable[len(onTable)-1]-1) && !majunFunc.Contain(playerList,onTable[len(onTable)-1]+1)) || onTable[len(onTable)-1]>40) && majunFunc.Count(playerList,onTable[len(onTable)-1])==3{
        fmt.Println("槓牌:%d",onTable[len(onTable)-1])
        ...
        *player=playerList
        ...
        fmt.Println("碰牌:%d",onTable[len(onTable)-1])
        eatCard(&playerList,1,onTable[len(onTable)-1])
        scoreList:=score(&playerList)
            
        fmt.Println("3%v",playerList)
        //fmt.Println("%v",scoreList)
        min:=majunFunc.Min(scoreList)
        for i,v :=range scoreList{
            if v==min{
...
func checkWin(player *[]int)int{
    ...
    if majunFunc.Contain(listen,onTable[len(onTable)-1]){
        fmt.Println("聽")
        fmt.Println("5%v",listen)
        fmt.Println("win")
        fmt.Println("%d",onTable[len(onTable)-1])
        return 0
    }else{
        return 1
    }
...
func playGame(player *[]int)int{
    ...
    if len(onTable)==0{    
        ...
        if len(listen)!=0{
            if majunFunc.Contain(listen,playerList[len(playerList)-1]){
                fmt.Println("聽")
                fmt.Println("6%v",listen)
                fmt.Println("win itself")
                fmt.Println("%d",takeCard)
                return 1
            }      
        }
        scoreList:=score(&playerList)
        
        fmt.Println("7%v",playerList)
        //fmt.Println("%v",scoreList)
        min:=majunFunc.Min(scoreList)
        for i,v :=range scoreList{
            if v==min{
    ...
    if len(listen)!=0{
        ...
        if majunFunc.Contain(listen,playerList[len(playerList)-1]){
            fmt.Println("聽")
            fmt.Println("8%v",listen)
            fmt.Println("win itself")
            fmt.Println("%d",takeCard)
            return 1
        }else{
            scoreList:=score(&playerList)
            fmt.Println("9%v",playerList)
            //fmt.Println("%v",scoreList)
            min:=majunFunc.Min(scoreList)
            for i,v :=range scoreList{
                if v==min{
...
func Play(){
	...
	for (len(cards)>8 && playFinish==0){
		...
		if len(onTable)!=0{
	        for j:=0;j<3;j++{
	            ...
	            if checkAAA(&player[(i+1)%4]){
	            	fmt.Println("player%d 碰",(i+1)%4+1)
	                i=(i+1+1)%4
	                continue
	            } 
	            if checkAAA(&player[(i+2)%4]){
	            	fmt.Println("player%d 碰",(i+2)%4+1)
	                i=(i+2+1)%4
	                continue
	            }
	            if checkAAA(&player[(i+3)%4]){
	            	fmt.Println("player%d 碰",(i+3)%4+1)
	                i=(i+3+1)%4
	                continue
	            }  
	        }
	    ...
	    fmt.Println("player%d",(i+1))
	    playFinish=playGame(&player[i])
	    fmt.Println("11%v",player[i])
	    if playFinish==1{
	    	winner=i+1
	        break
	    }   
	...
	if playFinish==1{
	    fmt.Println("winner: player%d",winner)
        fmt.Println(player[winner])
	}
